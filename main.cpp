...
class Bullet{
     ...
     Bullet(float radius, int id, float damage, int max_life){
          this->shape.setRadius(radius);
          this->id=id;
          this->damage=damage;
     ...
     void check(int max_allowed){
          this->counter++;
          this->can_erase = !this->can_erase?this->counter>max_allowed*this->max_life:this->can_erase;
     }
...
Enemy* collision_detection(Bullet v, std::vector<Enemy> &enemies){
  for(Enemy &e: enemies){
          ...
          if(distance < radi_sum*radi_sum){return &e;}
     }
...
void player_controls(sf::RenderWindow &window,std::vector<Bullet> &bullets, Player &player1){
     //SHOOT
     Bullet b1{1.1f,1};
     sf::Vector2f aim_dir;
...
int main(){
     ...
     while(window.isOpen()){
          ...
          window.clear();
               ...
               player_controls(window,bullets,player1);
